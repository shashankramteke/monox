...
SERVICES = ["api-gateway", "python-service"]
ROUTES = {
    "api-gateway": ["/api/quote", "/api/proxy-slow-quote", "/api/health", "/api/quotes/random", "/api/proxy-quote"],
    "python-service": ["/quote", "/slow-quote", "/quotes/random", "/quotes/search", "/health"],
}
SEVERITIES = ["DEBUG", "INFO", "INFO", "INFO", "WARNING", "ERROR"]
LOG_BODIES = {
    "api-gateway": [
        "Incoming request processed successfully",
        "Proxying request to python-service",
        ...
        "Cache miss for quote ID {id}",
        "Request tracing: span created trace_id={tid}",
        "Middleware pipeline completed in {dur}ms",
    ],
    "python-service": [
        "Quote fetched from database in {dur}ms",
        "Database query executed: SELECT * FROM quotes WHERE id={id}",
        "Cache hit for quote {id}, returning cached result",
        "Processing slow quote request with artificial delay",
        "[REDACTED_AUTHOR] quote attribution verified",
        "Database connection pool: size={n}, available={idle}",
        "Quote search query matched {n} results",
        "Random quote selected: id={id}",
        "Health check: database=OK, cache=OK, uptime={dur}s",
        "[REDACTED_EMAIL] user lookup for quote permissions",
    ],
}
    ...
    while True:
        try:
            ...
            for svc in SERVICES:
                ...
                # Redaction count (slow growth + occasional jumps)
